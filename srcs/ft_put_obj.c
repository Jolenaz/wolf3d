...
/*   Updated: 2016/02/25 09:42:41 by jbelless         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_cop_obj(t_pict pict, t_env *e)
{
	...
	c = (unsigned char*)e->data_obj[pict.tex] + pict.texx * 4
		+ pict.texy * 4 * SIZE_O;
...
static void	ft_obj1(int i, t_env *e, t_tmpo *tmpo)
{
	tmpo->xobj = e->tabobj[i].x - e->xcam + 0.5;
	tmpo->yobj = e->tabobj[i].y - e->ycam + 0.5;
	tmpo->invdev = 1.0 / (e->xscreen * e->ydir - e->xdir * e->yscreen);
	tmpo->transformx = tmpo->invdev *
		(e->ydir * tmpo->xobj - e->xdir * tmpo->yobj);
	tmpo->transformy = tmpo->invdev *
		(-e->yscreen * tmpo->xobj + e->xscreen * tmpo->yobj);
	tmpo->xscreenobj = (int)((SIZE_W / 2) *
			(1 + tmpo->transformx / tmpo->transformy));
...
static void	ft_obj2(int i, t_env *e, t_pict *pict, t_tmpo *tmpo)
{
	pict->y = tmpo->drawstarty;
	pict->texx = (int)(256 * (pict->x - (-tmpo->objwidth /
					2 + tmpo->xscreenobj)) * SIZE_O / tmpo->objwidth) / 256;
	if (tmpo->transformy > 0 && pict->x > 0 &&
			pict->x < SIZE_W && tmpo->transformy < e->zbuff[pict->x])
	{
